...
<script setup lang="ts" name="dvUploadImg">
...
// 图片裁剪组件
const dvCrooper = defineAsyncComponent(
    () => import('@/components/dvCropper/index.vue'),
);
...
interface UploadImageProps {
    ...
    autoUpload?: boolean; // 是否开启图片自动上传
    cropper?: boolean; // 是否开启图片裁剪
    compress?: number; //
    aspectRatio?: string;
}
...
const uploadChange: UploadProps['onChange'] = (file, files) => {
    ...
    if (props.cropper && file.status == 'ready') {
        const imgType = props.fileType.includes(
            file?.raw.type as File.ImageMimeType,
        );
        if (!imgType)
            ElNotification({
                title: '温馨提示',
